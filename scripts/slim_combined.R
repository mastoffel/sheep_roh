...
future_map(seeds, slim_roh,  genome_size = 1e8, pop_size1 = 2000, pop_size2 = 200, 
                  time1 = 10000, time2 = 11000, mut_rate_del = 1e-9, recomb_rate = 1.27e-8,
                  # mutationtype 1
                  mut1_dom_coeff = 0.05, mut1_gam_mean = -0.03, mut1_gam_shape = 0.5)
                  # mutationtype2
...
p1 <- ggplot(mut_p, aes(roh_class, s_sum_per_MB, fill = roh_class)) +
   ...
   ylab("selection coefficient per Mb") + 
...
p1
ggsave("figs/del_roh_weakly_1248.jpg", plot = p1, width = 6, height = 4)
