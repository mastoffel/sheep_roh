# plotting mutation characteristics across ROH classes
library(tidyverse)
library(ggplot2)
library(gghalves)
library(data.table)
...
p1 <- make_plot(s_sum_per_MB,"Mutation load\n(Selection coefficient per cM)", mut_sub)  
...
make_plot2 <- function(ss, axis_title, df) {
   ...
   ggplot(df, aes(roh_class, !!ss, fill = roh_class)) +
      ...
      #scale_y_continuous(breaks = seq(from = 0.015, to = 0.04, by = 0.005)) +
      ylab(axis_title) +
      theme(
         axis.line.y = element_blank(),
         axis.ticks.y = element_blank(), 
